...
public class myDB {
    ...
    public static ResultSet showBookings() {
        try {
            Connection conn = DriverManager.getConnection (CommonConstants.DB_URL);
            PreparedStatement showBookings = conn.prepareStatement ("SELECT bookings.flightid, flights.`from`, flights.`to`, flights.takeoffdate, bookings.tickettype " +
                    "FROM bookings " +
                    "JOIN flights ON bookings.flightid = flights.flightid " +
                    "WHERE bookings.bookerid = ?");
            showBookings.setInt (1, CommonConstants.CURRENT_USER_ID);
            ResultSet rs = showBookings.executeQuery ();
            if (!rs.isBeforeFirst ()) {
                return null;
            }
            return rs;
    ...
    public static String getFlightType(int userID,int flightID){
        try {
            Connection conn = DriverManager.getConnection (CommonConstants.DB_URL);
            PreparedStatement getFlightType = conn.prepareStatement ("SELECT type FROM passengers WHERE user=? AND flight=?");
            getFlightType.setInt (1,userID);
            getFlightType.setInt (2,flightID);
            ResultSet rs = getFlightType.executeQuery ();
            if (!rs.isBeforeFirst ()) return null;
            rs.next ();
    ...
    public static String getFlightResident(int userID,int flightID){
        try {
            Connection conn = DriverManager.getConnection (CommonConstants.DB_URL);
            PreparedStatement getFlightResident = conn.prepareStatement ("SELECT hotel FROM passengers WHERE user=? AND flight=?");
            getFlightResident.setInt (1,userID);
            getFlightResident.setInt (2,flightID);
            ResultSet rs = getFlightResident.executeQuery ();
            if (!rs.isBeforeFirst ()) return null;
            rs.next ();
            return rs.getString ("hotel");
    ...
    public static int getNoMeals(int userID,int flightID){
        try {
            Connection conn = DriverManager.getConnection (CommonConstants.DB_URL);
            PreparedStatement getNoMeals = conn.prepareStatement ("SELECT meals FROM passengers WHERE user=? AND flight=?");
            getNoMeals.setInt (1,userID);
            getNoMeals.setInt (2,flightID);
            ResultSet rs = getNoMeals.executeQuery ();
            if (!rs.isBeforeFirst ()) return -1;
            rs.next ();
            return rs.getInt ("meals");
